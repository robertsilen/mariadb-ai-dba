...
AI: Connects, checks for previous snapshots and daemon samples
  ↓
If snapshots exist: "Compare to latest / oldest / pick a different one / skip?"
...
|---|---|---|
| **performance** | Query throughput, statement mix, temp tables to disk, table locks | Daemon samples or snapshot history |
...
The read/write workload ratio is rendered as an inline SVG donut chart directly by the AI in the Executive Summary — it is not generated by graph.py.
...
| F6 | Read/write workload ratio | **IMPLEMENTED** | Inline SVG donut chart in Executive Summary |
...
| F6 | **Read/write workload ratio** — show the ratio of reads (`Com_select`) to writes (`Com_insert` + `Com_update` + `Com_delete` + `Com_replace`) as a donut chart at the top of the report. People commonly misjudge their workload — "my system is read-intensive" — but each read also generates redo log writes, and the actual DML mix often tells a different story. Present at the top of the Executive Summary to frame everything that follows | **Cumulative** (lifetime ratio from STATUS counters; delta ratio from comparison window is even more useful) | Lefred comment | **IMPLEMENTED** | Inline SVG donut chart written directly by AI in the Workload Profile subsection (not generated by graph.py). |
...
| J4 | **Reproducible methodology appendix** — ideally, include the actual SQL queries used for each analysis inline in the report so readers can re-run them independently. Current implementation links to the collector source code as a workaround to avoid sync issues between collect.py and the report template | **N/A** (report structure) | Lefred expertise | **IMPLEMENTED** (workaround) | Consulting reports show inline SQL for transparency. Inline queries would be optimal but require a mechanism to extract them from collect.py at report time without the token cost of reading the full source. Current workaround: link to source. |
